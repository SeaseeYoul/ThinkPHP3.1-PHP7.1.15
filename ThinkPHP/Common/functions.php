...
function halt($error) {
    ...
    report_error_email(MODULE_NAME,ACTION_NAME,is_array($error) ? $error['message'] : $error);
    // 包含异常页面模板
...
//添加报错模块的监控
function report_error_email($module,$action,$message){
    $action_arr = array('register_device','up_push_code','new_index','item','get_verify_code','signin','set_passwd','getUserInfo','updateUserInfo','new_apply','h5apply','callBack','livingBody','checkCarded');//定义提示的报错模块
    if(in_array($action,$action_arr)){
        ...
        if(!$cache->get($key)){
            ...
            unset($cache);
            ob_start();
            $hdenv = HDENV =='test'?'测试线':'正式线';
            $mail = new SendMail();
            $mail->setServer("smtp.exmail.qq.com", "monitor.yunwei@haodai.net", "Asdf@123", 465, true); //设置smtp服务器，到服务器的SSL连接
            $mail->setFrom("monitor.yunwei@haodai.net"); //设置发件人
            $mail->setReceiver("zhaoqingyang@haodai.net"); //设置收件人，多个收件人，调用多次
            $mail->setCc("caoxianfeng@haodai.net"); //设置抄送，多个抄送，调用多次
            $mail->setCc("yangkun@haodai.net"); //设置抄送，多个抄送，调用多次
            $mail->setMail('闪贷紧急【BUG】!!!'.date('Y-m-d H:i:s'),$hdenv.'错误位置:'.$module.'-->'.$action.'-->'.$message); //设置邮件主题、内容
            $mail->sendMail(); //发送
            ob_end_clean();
        }
    }
