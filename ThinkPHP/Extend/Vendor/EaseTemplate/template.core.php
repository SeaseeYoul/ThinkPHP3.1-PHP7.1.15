...
//			$ShowTPL = @preg_replace("/\";(.+?)echo\"/e", '$this->FixPHP(\'\\1\')', $ShowTPL);
        ...
        $temp = $this;
        $ShowTPL = preg_replace_callback("/\";(.+?)echo\"/", function($r) use( $temp ) {
            $temp->FixPHP($r[1]);
            }, $ShowTPL);
...
//				$ShowTPL = preg_replace("/\(T_T\)(.+?)\(T_T!\)/ise", '$this->FixPHP(\'\\1\')', $ShowTPL);
                $ShowTPL = preg_replace_callback("//\(T_T\)(.+?)\(T_T!\)/is", function($r) use( $temp ) {
                    $temp->FixPHP($r[1]);
                }, $ShowTPL);
			}
	...
	*/
	function inc_preg(
        ...
        $temp = $this;
//        $pase = preg_replace('/(\{\s*|<!--\s*)inc\:([^\{\} ]{1,100})(\s*\}|\s*-->)/eis', '$this->inc("\\2")', $content);
        $pase = preg_replace_callback("/(\{\s*|<!--\s*)inc\:([^\{\} ]{1,100})(\s*\}|\s*-->)/is", function($r) use( $temp ) {
            return $temp->inc($r[2]);
        }, $content);
//		return preg_replace('/<\!--\s*\#include\s*file\s*=(\"|\')([a-zA-Z0-9_\.\|]{1,100})(\"|\')\s*-->/eis', '$this->inc("\\2")', $pase);
        return preg_replace_callback('/<\!--\s*\#include\s*file\s*=(\"|\')([a-zA-Z0-9_\.\|]{1,100})(\"|\')\s*-->/is', function($r) use( $temp ) {
            return $temp->inc($r[2]);
        }, $pase);
	}
